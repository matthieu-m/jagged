...
impl Allocation {
    fn new(pointer: *mut u8, layout: Layout) -> Self {
        Allocation {
            size: layout.size(),
...
struct TestAllocator {
    ...
    allowed: cell::Cell<usize>,
    //  The allocations performed; to check deallocation requests.
    allocations: cell::RefCell<Vec<Allocation>>,
}

impl TestAllocator {
    fn allocations(&self) -> Vec<Allocation> {
...
struct SpyCount(AtomicUsize);

impl SpyCount {
    fn zero() -> Self { SpyCount(AtomicUsize::new(0)) }
