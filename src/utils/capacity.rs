...
//!
//! Apart from holding the capacity itself, assembles various computing primitives based off the capacity.
...
impl Capacity {
    ...
    pub fn of_bucket(self, bucket: BucketIndex) -> BucketCapacity {
        ...
        let multiplier = if bucket.0 == 0 { 1 } else { 1usize << (bucket.0 - 1) };
